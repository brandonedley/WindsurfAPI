...
function toolNameFromTool(tool) {
...
function primaryStringParam(tool) {
    if (typeof tool === 'string')
        return 'input';
    ...
    for (const key of required) {
        if (params.properties[key]?.type === 'string')
            return key;
    }
...
export function isRecoveryAllowed(_toolName, _paramName, _recoveryKind) {
    return false;
}
export function classifyHermesTool(tool, _modelPolicy = {}, _adapterOptions = {}) {
