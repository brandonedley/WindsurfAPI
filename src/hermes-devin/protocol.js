...
function declaredToolMap(tools) {
    const map = new Map();
...
function makeDiagnostics(requestId, model, provider, state, data = {}) {
    return {
        requestId,
        model,
        provider,
...
function validateArgumentsJson(args) {
...
function validateRequiredArguments(tool, value) {
...
function normalizedToolCall(call, index) {
...
function textLooksLikeToolNarration(text) {
