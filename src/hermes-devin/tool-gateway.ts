...
function buildCertification(classified, effectiveTools, inputTools) {
  ...
  for (const tool of classified) {
    if (tool.executionOwner) byOwner[tool.executionOwner] = (byOwner[tool.executionOwner] || 0) + 1;
    else unknown.push(tool.name || '(unnamed)');
  }
...
export function buildHermesDevinToolGateway(tools = [], options = {}) {
  ...
  for (const tool of Array.isArray(tools) ? tools : []) {
    ...
    if (routed.mode === TOOL_MODE.native) native.push(routed);
    else if (routed.mode === TOOL_MODE.denied) denied.push(routed);
    else emulated.push(routed);
  }
